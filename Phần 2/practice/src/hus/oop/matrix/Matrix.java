...
public class Matrix {
    ...
    private void initRandom(int rows, int columns) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                this.data[i][j] = Math.round((Math.random() * 10) * 100) / 100;
            }
        }
    }
    ...
    private boolean isChecked(Matrix another) {
    ...
    public Matrix multiply(Matrix that) {
        /* TODO */
        double[][] currentMatrix = new double[this.data.length][that.getData()[0].length];
        for (int i = 0; i < currentMatrix.length; i++) {
            for (int j = 0; j < currentMatrix[i].length; j++) {
                int temp = 0;

                for (int k = 0; k < this.data[0].length; k++) {
                    temp += this.data[i][k] * that.get(k, j);
                }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(matrix.add(matrix1));
        System.out.println(matrix.multiply(matrix1.transpose()));
    }
}
