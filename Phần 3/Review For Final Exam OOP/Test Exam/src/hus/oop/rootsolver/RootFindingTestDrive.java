...
public class RootFindingTestDrive {
    ...
    public static void testRootSolver() {
         ...
         */
        System.out.println("sin(x).x - 3 = 0");
        UnivariateRealRootFinding univariateRealRootFinding =
                new UnivariateRealRootFinding(new UnivariateRealFunction(),
                        new BisectionSolver(0.001, 100));

        System.out.println("using Bisection Solver x = "
                + univariateRealRootFinding.solve(12, 14));

        univariateRealRootFinding.setRootSolver(new SecantSolver(0.001, 100));

        System.out.println("using Secant Solver x = "
                + univariateRealRootFinding.solve(12, 14));

        univariateRealRootFinding.setRootSolver(new NewtonRaphsonSolver(0.001, 100));

        System.out.println("using Newton Raphson Solver x = "
                + univariateRealRootFinding.solve(12, 14));
    }
}
